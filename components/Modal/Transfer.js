...
const Transfer = ({
  ...
  const [activeThirdwebTokens, setActiveThirdWebToken] = useState();
  ...
  useEffect(() => {
    const url = imageUrlBuilder(client).image(selectedToken.logo.asset).url();
    setImageUrl(url);
  }, [selectedToken]);

  useEffect(() => {
    const getBalance = async () => {
      const balance = await activeThirdwebTokens.balanceOf(sender);
